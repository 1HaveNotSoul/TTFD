...
async def check_auto_update(bot):
    ...
    if auto_update_info.get('enabled') and auto_update_info.get('changes'):
        ...
        # Отправляем обновление со ВСЕМИ изменениями
        success = await send_update_notification(
            bot=bot,
            changes=all_changes,  # Отправляем все изменения сразу
            major=False
        )
        
        if success:
            print(f"✅ Уведомление об обновлении отправлено в #📄・ɴᴏᴛɪꜰɪᴄᴀᴛɪᴏɴs")
